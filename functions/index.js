...
async function fetchRates() {
    ...
    for (let i = 0; i < currencies.length; i++) {
        let code = currencies[i].id
        let url = "https://api.exchangerate.host/latest?base="+code
        ...
        promise.then((response) => {
            ...
            for (let c in response.data.rates) {
                rates[c.toLowerCase()] = response.data.rates[c]
            }

            admin.firestore().doc("rates/"+code).set(rates)
        })
    }
